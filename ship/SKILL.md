...
- **`../review/SKILL.md`** — both its `/review` command (diff review, composing `../develop` and
  its own bundled `./skills/` copy of Ponytail, scored per `../review/references/RISK.md`) and its
  `/plan` command (pseudocode-level planning under the same two rulesets). `/ship` never reads
  `review/skills/` itself — only `/review`/`/plan` do — so this is descriptive, not a path `/ship`
  follows.
